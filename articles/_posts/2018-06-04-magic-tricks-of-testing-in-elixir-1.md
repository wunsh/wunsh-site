...
***Примечание:** данный GenServer не содержит реализаций функций взаимодействия с клиентом (`handle_call`), потому что в этом примере GenServer используется напрямую (9 строчка в приведённом ниже коде).*
